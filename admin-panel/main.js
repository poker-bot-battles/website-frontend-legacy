...
function makeTableDiv(table, tableName) {
  ...
  const bots = makeElement(
    "p",
    "",
    "Bots: " + table.filter((file) => !file.endsWith(".json")).length + "/10",
  );
  ...
  for (const filename of table) {
    ...
    let moveBtn = makeElement("button", "", "Move", () =>
      popup(tableName, filename),
    );
    let deleteBtn = makeElement("button", "", "Delete", () =>
      deleteFile(tableName, filename),
    );
  ...
  const deleteResultBtn = makeElement("button", "", "Delete Result File", () =>
    deleteFile(tableName, `table-${tableName}.json`),
  );
